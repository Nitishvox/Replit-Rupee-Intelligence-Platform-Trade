...
# Verify database_manager.py exists
if not os.path.exists(os.path.join('utils', 'database_manager.py')):
    st.error("Error: utils/database_manager.py not found. Please ensure the file exists in the utils directory.")
    st.stop()
            ...
            if st.button("🏗️ Generate Accounts", type="primary", use_container_width=True):
                with st.spinner("Creating SRVA accounts..."):
                    try:
                        ...
                        st.error(f"❌ Failed to generate accounts: {e}")
