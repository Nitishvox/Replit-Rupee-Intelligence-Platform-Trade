...
class SRVAManager:
    ...
    def load_reference_data(self):
        ...
        try:
            # Load countries data with UTF-8 encoding
            with open('data/countries.json', 'r', encoding='utf-8') as f:
                self.countries_data = json.load(f)
            # Load banks data with UTF-8 encoding
            ...
            st.error(f"Error decoding JSON reference data: {str(e)}")
            # Log file content preview for debugging
            try:
                with open('data/banks.json', 'r', encoding='utf-8') as f:
                    content = f.read(500)  # Read first 500 characters
                    st.write("Banks JSON content preview:", content)
                with open('data/countries.json', 'r', encoding='utf-8') as f:
                    content = f.read(500)  # Read first 500 characters
                    st.write("Countries JSON content preview:", content)
            except Exception as inner_e:
                st.error(f"Failed to read file for preview: {str(inner_e)}")
            self.countries_data = {}
            self.banks_data = {}
